...
- 装飾用のSVGアイコンに `aria-hidden="true"` を設定し、スクリーンリーダーへの不要な読み上げを防いでいる
- 自己紹介ページの Contact セクション（Email / GitHub / X）をクリック可能なリンクにした
- テーマ切替ボタンの `aria-label` を、現在のテーマに合わせて動的に更新するようにした（ダークモード時は「ライトモード切替」、ライトモード時は「ダークモード切替」）
- テーマ切替・ACCEPT MISSION・COMPLETE の各ボタンに `:focus-visible` を追加し、Tab キーでフォーカスしたときにアウトラインが表示されるようにした
...
7. **EXPORT** を押すと、保存済みのログをまとめてMarkdownファイルとして書き出せる（ログは日付の古い順に並んで出力される）
